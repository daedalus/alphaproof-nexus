...
/-
  Population Member 06: Strategy = GeometricTiling with subset_T + cover_T
  Approach: Define a GeometricTiling structure that adds `subset_T` (all piece
    vertices are inside T, via barycentric coordinates) and `cover_T` (T's vertices
    are among the piece vertices). This rules out the trivial `tilable_all_pos`
    construction. Prove the equilateral grid construction satisfies the new
    constraints, making `squares_constructive` genuinely geometric.
  Rating (initial): Elo 1600
-/
...
-- The following three lemmas require a stronger definition (GeometricTiling with
-- disjointness and covering). They are not provable under the weak Tiling definition.
-- See member_05 for discussion.
