...
noncomputable def equilateral : Triangle where
  A := (0, 0)
  B := (1, 0)
  C := (1/2, Real.sqrt 3 / 2)
  nondegenerate := by
...
noncomputable def grid_triangles (k : ℕ) : Finset Triangle :=
  ...
  (Finset.product (Finset.range k) (Finset.range k)).image (λ ⟨i, j⟩ =>
    { A := ((i : ℝ) / k, (j : ℝ) * Real.sqrt 3 / (2 * k))
      ...
      nondegenerate := by
        -- The area is nonzero because the points are not collinear
        sorry })
