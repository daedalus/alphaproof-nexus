...
/-
  Population Member 08: Strategy = Area-scaling lemma + square classification
  Approach: Add the area-scaling lemma `geometric_area_scaling` showing
    n * signed_area(S) = signed_area(T) for any GeometricTiling T S n.
    Use this to prove that when both T and S are equilateral, n must be
    a perfect square (equilateral_tiling_implies_square). This settles
    the obstruction for the equilateral-piece case: 7 and 11 are not squares,
    hence not tilable by equilateral pieces.
    The general case (arbitrary S) remains open (3 sorries).
  Rating (initial): Elo 1700
-/
